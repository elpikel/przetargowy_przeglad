defmodule PrzetargowyPrzeglad.Api.BzpClient do
  ...
  defp parse_tender(raw) do
    %{
      ...
      submission_deadline:
        parse_datetime(
          raw["submittingOffersDate"] || raw["submissionDeadline"] || raw["tenderDeadline"]
        ),
  ...
  defp extract_cpv_codes(raw) do
    cond do
      ...
      is_list(raw["cpv"]) -> Enum.map(raw["cpv"], & &1["code"]) |> Enum.reject(&is_nil/1)
      true -> []
    end
  end
  ...
  defp parse_decimal(v) when is_binary(v) do
    cleaned = String.replace(v, ~r/[^\d.,]/, "") |> String.replace(",", ".")
